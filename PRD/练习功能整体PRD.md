...
| 2 | 模拟练习新增/编辑弹窗 | PC 后台 Web | 新增 | 新增/草稿编辑：返回、保存（草稿）；已发布（未开始/进行中）不再支持编辑，也不提供行操作；全部状态均提供「详情」按钮，打开只读表单页 |
...
| 创建人 | 创建者姓名 |
| 创建时间 | 年-月-日 时:分:秒 |
| 操作 | 全部状态：详情<br>草稿：详情 + 发布 + 编辑 + 删除<br>未开始 / 进行中 / 已结束：详情 |
...
| 2 | PC | 新增 | 练习设置 | 模拟练习 Tab | 模拟练习列表 | 展示所有练习配置列表（名称、时间、状态、创建人）。操作：全部状态→详情；草稿→详情+发布+编辑+删除；未开始/进行中/已结束→详情 |
| 3 | PC | 新增 | 练习设置 | 模拟练习 Tab | 新增/编辑/查看弹窗 | 弹窗分为4个分组：①基本信息（练习名称≤20字、练习开放时间范围）；②选择试卷（试卷选择器，草稿可改、已发布置灰，选择后信息栏展示试卷结构）；③练习题目数量（题目总数+各题型数量，未选择试卷前全部禁用，选择后自动启用并设置上限，草稿可编辑、已发布置灰）；④练习人员（人员多选弹窗，草稿编辑时完整回显已选人员/部门标签）。新增/草稿编辑底部：返回+保存；已发布状态不提供编辑入口；全部状态提供「详情」按钮，打开只读表单页（全部字段禁用），底部仅「返回」。校验规则11条依次执行 |
